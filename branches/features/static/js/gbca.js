...
function setGBCADataSeason(space, ctype, season) {
    if (!GBCA[space]) GBCA[space] = {
        "clo": {
            "spring": "",
            "summer": "",
            "fall": "",
    ...
    if (model == "pmvElevatedAirspeed") {
        GBCA[space].clo[season] = d.clo;
        GBCA[space].met[season] = d.met;
        GBCA[space].top[ctype][season] = !isCelsius ? util.CtoF(t_op).toFixed(1) : t_op.toFixed(1);
        GBCA[space].vel[ctype][season] = !isCelsius ? (d.vel * 196.9).toFixed(1) : d.vel.toFixed(1);
        GBCA[space].rh[ctype][season] = d.rh.toFixed(1);
        GBCA[space].pmv[ctype][season] = $("#pmv-res").html();
        GBCA[space].ppd[ctype][season] = $("#ppd-res").html();
        GBCA[space].comply[ctype][season] = comply ? "YesTest" : "No";
        GBCA[space].psychart[ctype][season] = document.getElementById('chart-div').innerHTML;
        GBCA[space].temphumchart[ctype][season] = document.getElementById('temphumchart-div').innerHTML;
    } else {
        GBCA[space]["adaptchart"][season] = document.getElementById('chart-div-adaptive').innerHTML;
    }
}

function writeGBCARowClo(spacetype, windowref) {

    var space = GBCA[spacetype];
...
function writeGBCARowAmb(spacetype, hc, windowref) {
...
function writeGBCARowPmv(spacetype, hc, windowref) {
    var space = GBCA[spacetype]
    var newcell = "</td><td class='value-cell'>"
...
function insertGBCACharts(spacetype, hc, windowref) {
    var space = GBCA[spacetype]
...
function insertGBCAAdaptCharts(spacetype, windowref) {
    var space = GBCA[spacetype]
    var table = windowref.document.getElementById("GBCA-adapt-charts")
    var seasons = ["spring", "summer", "fall", "winter"]
...
function generateGBCATables(windowref) {
    for (var key in GBCA) {
        var chart = GBCA[key].psychart
        var temphumchart = GBCA[key].temphumchart
        var adaptchart = GBCA[key].adaptchart
        ...
        if (cool_test || heat_test || temphum_cool_test || temphum_heat_test) {
            writeGBCARowClo(key, windowref);
        }

        if (cool_test || temphum_cool_test) {
            writeGBCARowAmb(key, "cooling", windowref);
            writeGBCARowPmv(key, "cooling", windowref);
            insertGBCACharts(key, "cooling", windowref);
        }
        if (heat_test || temphum_heat_test) {
            writeGBCARowAmb(key, "heating", windowref);
            writeGBCARowPmv(key, "heating", windowref);
            insertGBCACharts(key, "heating", windowref);
        }
        if (adapt_test) {
            insertGBCAAdaptCharts(key, windowref);
        }
    }
}

function createGBCADocument(html) {
    var doc = document.implementation.createHTMLDocument('');
    doc.body.innerHTML = html;
    return doc
}

function openGBCADocument(doc) {
