...
function setDataSeason(space, ctype, season) {
  ...
  if (model == "pmvElevatedAirspeed") {
    ...
    LEED[space].top[ctype][season] = !isCelsius
      ? util.CtoF(t_op).toFixed(1)
      : t_op.toFixed(1);
    LEED[space].vel[ctype][season] = !isCelsius
      ? (d.vel * 196.9).toFixed(1)
      : d.vel.toFixed(1);
    ...
    LEED[space].psychart[ctype][season] = document.getElementById(
      "chart-div"
    ).innerHTML;
    LEED[space].temphumchart[ctype][season] = document.getElementById(
      "temphumchart-div"
    ).innerHTML;
  } else {
    LEED[space]["adaptchart"][season] = document.getElementById(
      "chart-div-adaptive"
    ).innerHTML;
  }
}
...
function writeRowPmv(spacetype, hc, windowref) {
  ...
  newRow.innerHTML =
    ...
    space.ppd[hc].winter +
    newcell +
    space.comply[hc].spring +
    newcell +
    space.comply[hc].summer +
    newcell +
    space.comply[hc].fall +
    newcell +
    space.comply[hc].winter +
    "</td>";
