...
(function ($) {
  $.widget("ui.selectmenu", {
    ...
    _create: function () {
      ...
      this.newelement = $("<a />", {
        class:
          this.widgetBaseClass + " ui-widget ui-state-default ui-corner-all",
        id: this.ids[1],
        role: "button",
        href: "#nogo",
      ...
      this.newelement.prepend(
        '<span class="' + self.widgetBaseClass + '-status" />'
      );
      ...
      $(window).bind(
        "resize.selectmenu-" + this.ids[0],
        $.proxy(self.close, this)
      );
    },
    ...
    _init: function () {
      ...
      if (selectOptionData.length) {
        for (var i = 0; i < selectOptionData.length; i++) {
              ...
              if (
                ...
                !self._disabled(
                  $(event.currentTarget).parents(
                    "ul>li." + self.widgetBaseClass + "-group "
                  )
                )
              ) {
          ...
          if (selectOptionData[i].parentOptGroup.length) {
            var optGroupName =
              ...
              this.element
                .find("optgroup")
                .index(selectOptionData[i].parentOptGroup);
            ...
            for (var j in o.icons) {
              if (thisLi.is(o.icons[j].find)) {
                ...
                if (selectOptionData[i].bgImage) {
                  thisLi
                    .find("span")
                    .css("background-image", selectOptionData[i].bgImage);
                }
              }
            }
          }
        ...
        $(
          '<li role="presentation"><a href="#nogo" tabindex="-1" role="option"></a></li>'
        ).appendTo(this.list);
      }
      ...
      this.list
        .toggleClass(
          self.widgetBaseClass + "-menu-dropdown ui-corner-bottom",
          isDropDown
        )
        .toggleClass(
          self.widgetBaseClass + "-menu-popup ui-corner-all",
          !isDropDown
        )
      ...
      this._optionLis = this.list.find(
        "li:not(." + self.widgetBaseClass + "-group)"
      );
    ...
    _typeAhead: function (code, eventType) {
      ...
      self._typeAhead_chars = (self._typeAhead_chars === undefined
        ? ""
    ...
    _moveSelection: function (amt, recIndex) {
      ...
      if (!this.options.disabled) {
        var currIndex = parseInt(
          this._selectedOptionLi().data("index") || 0,
          10
        );
        ...
        if (
          this._optionLis
            .eq(newIndex)
            .hasClass(this.namespace + "-state-disabled")
        ) {
    ...
    _moveFocus: function (amt, recIndex) {
      ...
      var activeID =
        this.widgetBaseClass + "-item-" + Math.round(Math.random() * 1000);
    ...
    _scrollPage: function (direction) {
      var numPerPage = Math.floor(
        this.list.outerHeight() / this.list.find("li:first").outerHeight()
      );
    ...
    _disableOptgroup: function (index) {
      var optGroupElem = this.list.find(
        "li." + this.widgetBaseClass + "-group-" + index
      );
      if (optGroupElem) {
        optGroupElem
          .addClass(this.namespace + "-state-disabled")
